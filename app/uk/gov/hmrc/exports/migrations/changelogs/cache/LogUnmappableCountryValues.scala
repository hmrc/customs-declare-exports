...
import org.mongodb.scala.model.Filters.{exists, or}
import play.api.Logging
import play.api.libs.json.{JsArray, JsString, Json}
import uk.gov.hmrc.exports.migrations.changelogs.cache.LogUnmappableCountryValues.{allCountries, getCountryCode}
...
class LogUnmappableCountryValues extends MigrationDefinition with Logging {

  private val batchSize = 100

  override val migrationInformation: MigrationInformation =
    MigrationInformation(
      id = s"CEDS-5773 Find unmappable country values in database to include in later migration",
      order = 26,
      author = "Tom Robinson"
    )
  ...
  override def migrationFunction(db: MongoDatabase): Unit = {
    ...
    val commonFieldName = "details.address.country"
    val detailTypes = Seq("exporterDetails", "consigneeDetails", "declarantDetails", "representativeDetails", "carrierDetails", "consignorDetails")
    ...
    declarationCollection
      ...
      .batchSize(batchSize)
      .asScala
      .foreach(logUnmappableValues)

    def logUnmappableValues(declaration: Document): Unit = {
      @tailrec
      def getFieldValue(doc: Document, path: List[String]): Seq[String] = path match {
        case head :: Nil                                     => Seq(doc.get(head, classOf[String]))
        case head :: tail if Option(doc.get(head)).isDefined => getFieldValue(doc.get(head, classOf[Document]), tail)
        case _                                               => Nil
      }
      val countryValues: Seq[String] = detailTypes.flatMap { detailType =>
        val commonPath: Array[String] = commonFieldName.split("\\.")
        getFieldValue(declaration, "parties" :: detailType :: commonPath.toList)
      }

      countryValues.foreach { value =>
        getCountryCode(value) match {
          case Some(_)                                                 => ()
          case None if allCountries.map(_.countryCode).contains(value) => ()
          case _                                                       => logger.warn(s"Unable to find country code for [$value]")
        }
      }
    }
...
object LogUnmappableCountryValues {
