...
class RenameAndConvertBorderTransportNationalityField extends MigrationDefinition with Logging {
  ...
  override def migrationFunction(db: MongoDatabase): Unit = {
    ...
    def updateTransportCrossingTheBorderNationality(declaration: Document): Document = {
      ...
      val updatedDec = borderTransportData.map { data =>
        RenameAndConvertBorderTransportNationalityField.getCountryCode(data) match {
          case Some(code) => renameFieldWithCountryCode(code)
          case None if RenameAndConvertBorderTransportNationalityField.allCountries.map(_.countryCode).contains(data) =>
...
object RenameAndConvertBorderTransportNationalityField {
