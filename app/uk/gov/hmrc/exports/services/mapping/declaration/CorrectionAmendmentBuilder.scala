...
class CorrectionAmendmentBuilder @Inject() () extends ModifyingBuilder[Seq[ExportsFieldPointer], Declaration] {

  override def buildThenAdd(alteredFields: Seq[ExportsFieldPointer], declaration: Declaration): Unit = {
    val amendment = new Declaration.Amendment()

    // val pointer = new Declaration.Amendment.Pointer()
