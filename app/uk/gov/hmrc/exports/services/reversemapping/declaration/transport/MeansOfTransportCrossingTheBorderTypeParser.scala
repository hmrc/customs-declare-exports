...
@Singleton
class MeansOfTransportCrossingTheBorderTypeParser extends DeclarationXmlParser[Option[String]] {

  override def parse(inputXml: NodeSeq)(implicit context: MappingContext): XmlParserResult[Option[String]] =
    Right(StringOption((inputXml \ Declaration \ BorderTransportMeans \ IdentificationTypeCode).text))
}
