...
@Singleton
class RoutingCountriesParser extends DeclarationXmlParser[Seq[Country]] {

  override def parse(inputXml: NodeSeq)(implicit context: MappingContext): XmlParserResult[Seq[Country]] = Right(
    (inputXml \ Declaration \ Consignment \ Itinerary \ RoutingCountryCode)
      .flatMap(_.toStringOption)
