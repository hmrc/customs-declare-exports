...
@Singleton
class MovementsRepository @Inject()(implicit mc: ReactiveMongoComponent,
                                    ec: ExecutionContext)
  ...
  override def indexes: Seq[Index] = Seq(
    ...
    Index(Seq("ducr" -> IndexType.Ascending),
          unique = true,
          name = Some("ducrIdx"))
  )
  ...
  def getByConversationId(
      conversationId: String): Future[Option[MovementSubmissions]] =
    find("conversationId" -> JsString(conversationId)).map(_.headOption)

  def getByEoriAndDucr(eori: String,
                       ducr: String): Future[Option[MovementSubmissions]] =
    find("eori" -> JsString(eori), "ducr" -> JsString(ducr)).map(_.headOption)

  override def isInsertion(newRecordId: BSONObjectID,
                           oldRecord: MovementSubmissions): Boolean =
  ...
  def save(movementSubmission: MovementSubmissions) =
    insert(movementSubmission).map { res =>
      if (!res.ok)
        ...
        Logger.error(
          "Error during inserting movement result " + res.writeErrors.mkString(
            "--"))
      // $COVERAGE-ON$
      res.ok
    }
  ...
  def updateMovementStatus(movementSubmission: MovementSubmissions) = {
    val finder = BSONDocument(
      "_id" -> movementSubmission.id,
      "conversationId" -> movementSubmission.conversationId)

    val modifier = BSONDocument(
      "$set" -> BSONDocument("status" -> movementSubmission.status))
