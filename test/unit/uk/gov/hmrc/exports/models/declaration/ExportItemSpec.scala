...
class ExportItemSpec extends UnitSpec {
  "ExportItem.createDiff" should {
    ...
    "produce the expected ExportsDeclarationDiff instance" when {
      ...
      "the original version's procedureCodes field has a different value to this one" in {
        val fieldPointer = s"${baseFieldPointer}.${ProcedureCodes.pointer}"
        val item = ExportItem("latestId", procedureCodes = Some(ProcedureCodes(None, Seq.empty)))
      ...
      "the original version's additionalFiscalReferencesData field has a different value to this one" in {
        val fieldPointer = s"${baseFieldPointer}.${AdditionalFiscalReferences.pointer}"
        val item = ExportItem("latestId", additionalFiscalReferencesData = Some(AdditionalFiscalReferences(Seq.empty)))
        val originalValue = None
      ...
      "the original version's additionalInformation field has a different value to this one" in {
        val additionalInformations = AdditionalInformations(None, Seq(AdditionalInformation("latestCode", "latestDescription")))
        val fieldPointer = s"${baseFieldPointer}.${AdditionalInformations.pointer}.${AdditionalInformations.itemsPointer}.1"
  ...
  "ProcedureCodes.createDiff" should {
    val baseFieldPointer = ProcedureCodes.pointer

    "produce the expected ExportsDeclarationDiff instance" when {
      "no differences exist between the two versions" in {
        withClue("both values are empty") {
          val procedureCodes = ProcedureCodes(None, Seq.empty)
          procedureCodes.createDiff(procedureCodes, baseFieldPointer) mustBe Seq.empty
        }

        withClue("procedureCode is None, additionalProcedureCodes has some values") {
          val procedureCodes = ProcedureCodes(None, Seq("apc1"))
          procedureCodes.createDiff(procedureCodes, baseFieldPointer) mustBe Seq.empty
        }

        withClue("procedureCode is Some, additionalProcedureCodes is empty") {
          val procedureCodes = ProcedureCodes(Some("pc1"), Seq.empty)
          procedureCodes.createDiff(procedureCodes, baseFieldPointer) mustBe Seq.empty
        }

        withClue("procedureCode is Some, additionalProcedureCodes has some values") {
          val procedureCodes = ProcedureCodes(Some("pc1"), Seq("apc1", "apc2"))
          procedureCodes.createDiff(procedureCodes, baseFieldPointer) mustBe Seq.empty
        }
      }

      "the original version's procedureCode field has a different value to this one" in {
        val fieldPointer = s"${baseFieldPointer}.${ProcedureCodes.procedureCodesPointer}"
        val procedureCodes = ProcedureCodes(Some("pc1"), Seq("apc1", "apc2"))
      ...
      "when additionalProcedureCodes are present but not equal" in {
        val fieldPointer = s"$baseFieldPointer.${ProcedureCodes.additionalProcedureCodesPointer}"
        withClue("original procedureCode's additionalProcedureCodes are not present") {
          val procedureCodes = ProcedureCodes(None, apcs)
        ...
        withClue("both procedureCode additionalProcedureCodes contain different number of elements") {
          val procedureCodes = ProcedureCodes(None, apcs.drop(1))
          procedureCodes.createDiff(procedureCodes.copy(additionalProcedureCodes = apcs), baseFieldPointer) mustBe Seq(
        ...
        withClue("procedureCode's additionalProcedureCodes contain elements with different values") {
          val procedureCodes = ProcedureCodes(None, Seq("apc4") ++ apcs.drop(1))
          procedureCodes.createDiff(procedureCodes.copy(additionalProcedureCodes = apcs), baseFieldPointer) mustBe Seq(
  ...
  "AdditionalFiscalReferences.createDiff" should {
    ...
    "produce the expected ExportsDeclarationDiff instance" when {
      "no differences exist between the two versions" in {
        val additionalFiscalReferences = AdditionalFiscalReferences(Seq.empty)
      ...
      "when references are present but not equal" in {
        val fieldPointer = s"$baseFieldPointer.1.${AdditionalFiscalReferences.pointer}"
        withClue("original AdditionalFiscalReferences references are not present") {
          val additionalFiscalReferences = AdditionalFiscalReferences(references)
        ...
        withClue("both AdditionalFiscalReferences references contain different number of elements") {
          val additionalFiscalReferences = AdditionalFiscalReferences(references.drop(1))
          additionalFiscalReferences.createDiff(additionalFiscalReferences.copy(references = references), baseFieldPointer, Some(1)) mustBe Seq(
        ...
        withClue("container seals contain elements with different values") {
          ...
          val additionalFiscalReferences = AdditionalFiscalReferences(Seq(newValue) ++ references.drop(1))
          additionalFiscalReferences.createDiff(additionalFiscalReferences.copy(references = references), baseFieldPointer, Some(1)) mustBe Seq(
  ...
  "AdditionalInformations.createDiff" should {
    val baseFieldPointer = AdditionalInformations.pointer

    "produce the expected ExportsDeclarationDiff instance" when {
      "no differences exist between the two versions" in {
        val additionalInformations = AdditionalInformations(None, Seq.empty)
      ...
      "when items are present but not equal" in {
        val fieldPointer = s"$baseFieldPointer.${AdditionalInformations.itemsPointer}"
        withClue("original AdditionalInformations items are not present") {
          val additionalInformations = AdditionalInformations(None, items)
        ...
        withClue("both AdditionalInformations items contain different number of elements") {
          val additionalInformations = AdditionalInformations(None, items.drop(1))
          additionalInformations.createDiff(additionalInformations.copy(items = items), baseFieldPointer) mustBe Seq(
        ...
        withClue("AdditionalInformations items contain elements with different values") {
          ...
          val additionalInformations = AdditionalInformations(None, Seq(newValue) ++ items.drop(1))
          additionalInformations.createDiff(additionalInformations.copy(items = items), baseFieldPointer) mustBe Seq(
