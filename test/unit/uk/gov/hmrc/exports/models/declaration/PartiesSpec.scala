...
class PartiesSpec extends UnitSpec {
    ...
    "produce the expected ExportsDeclarationDiff instance" when {
      "no differences exist between the two versions" in {
        ...
        withClue("eori is Some, address is None") {
          val details = EntityDetails(Some("latestEori"), None)
          details.createDiff(details, baseFieldPointer) mustBe Seq.empty[AlteredField]
        }
        ...
        withClue("eori is Some, address is Some") {
          val details = EntityDetails(Some("latestEori"), Some(address))
          details.createDiff(details, baseFieldPointer) mustBe Seq.empty[AlteredField]
        }
      }
  ...
  "Parties.createDiff" should {
    ...
    "produce the expected ExportsDeclarationDiff instance" when {
      ...
      "the original version's declarationAdditionalActorsData field has a different value to this one" in {
        ...
        val originalValue = Some(DeclarationAdditionalActors(Seq.empty))
  ...
  "ExporterDetails.createDiff" should {
    ...
    "produce the expected ExportsDeclarationDiff instance" when {
      ...
      "the original version's eori field has a different value to this one" in {
        ...
        val exporterDetails = ExporterDetails(details)
        val originalValue = details.copy(eori = Some("original"))
  ...
  "DeclarationAdditionalActor.createDiff" should {
    val baseFieldPointer = DeclarationAdditionalActor.pointer

    "produce the expected ExportsDeclarationDiff instance" when {
      "no differences exist between the two versions" in {
        val declarationAdditionalActor = DeclarationAdditionalActor(None, None)
        declarationAdditionalActor.createDiff(declarationAdditionalActor, baseFieldPointer) mustBe Seq.empty[AlteredField]
      }

      "the original version's eori field has a different value to this one" in {
        val fieldPointer = s"${baseFieldPointer}.${DeclarationAdditionalActor.eoriPointer}"
        val declarationAdditionalActor = DeclarationAdditionalActor(Some("latestEori"), Some("latestPartyType"))
        val originalValue = Some("original")
      ...
      "the original version's partyType field has a different value to this one" in {
        val fieldPointer = s"${baseFieldPointer}.${DeclarationAdditionalActor.partyTypePointer}"
        val declarationAdditionalActor = DeclarationAdditionalActor(Some("latestEori"), Some("latestPartyType"))
        val originalValue = Some("original")
  ...
  "DeclarationAdditionalActors.createDiff" should {
    val baseFieldPointer = DeclarationAdditionalActors.pointer
    val actors = Seq(
      DeclarationAdditionalActor(Some("latestEoriOne"), Some("latestPartyTypeOne")),
      DeclarationAdditionalActor(Some("latestEoriTwo"), Some("latestPartyTypeTwo")),
      DeclarationAdditionalActor(Some("latestEoriThree"), Some("latestPartyTypeThree"))
    )

    "produce the expected ExportsDeclarationDiff instance" when {
      "no differences exist between the two versions" in {
        withClue("when no actors are present") {
          val decActors = DeclarationAdditionalActors(Seq.empty)
          decActors.createDiff(decActors, baseFieldPointer, Some(1)) mustBe Seq.empty[AlteredField]
        }

        withClue("when actors are present") {
          val decActors = DeclarationAdditionalActors(actors)
          decActors.createDiff(decActors, baseFieldPointer, Some(1)) mustBe Seq.empty[AlteredField]
        }
      }

      "when actors are present but not equal" in {
        val fieldPointer = s"$baseFieldPointer.${DeclarationAdditionalActor.pointer}"
        ...
        withClue("both container seals contain different number of elements") {
          val decActors = DeclarationAdditionalActors(actors.drop(1))
          decActors.createDiff(decActors.copy(actors = actors), baseFieldPointer) mustBe Seq(
        ...
        withClue("both container seals contain same elements but in different order") {
          val decActors = DeclarationAdditionalActors(actors.reverse)
          decActors.createDiff(decActors.copy(actors = actors), baseFieldPointer) mustBe Seq(
        ...
        withClue("container seals contain elements with different values") {
          val otherVal = "other"
          val decActors = DeclarationAdditionalActors(Seq(DeclarationAdditionalActor(Some(otherVal), Some(otherVal))) ++ actors.drop(1))
          decActors.createDiff(decActors.copy(actors = actors), baseFieldPointer) mustBe Seq(
            constructAlteredField(s"${fieldPointer}.eori", actors(0).eori, Some(otherVal)),
  ...
  "DeclarationHolder.createDiff" should {
    ...
    "produce the expected ExportsDeclarationDiff instance" when {
      ...
      "the original version's authorisationTypeCode field has a different value to this one" in {
        ...
        val declarationHolder = DeclarationHolder(Some("latestAuthorisationTypeCode"), Some("latestEori"), Some(EoriSource.UserEori))
      ...
      "the original version's eori field has a different value to this one" in {
        ...
        val declarationHolder = DeclarationHolder(Some("latestAuthorisationTypeCode"), Some("latestEori"), Some(EoriSource.UserEori))
        declarationHolder.createDiff(declarationHolder.copy(eori = originalValue), baseFieldPointer) mustBe Seq(
          constructAlteredField(fieldPointer, originalValue, declarationHolder.eori)
        )
      }
    }
  }
  ...
  "DeclarationHolders.createDiff" should {
    val baseFieldPointer = s"${DeclarationHolders.pointer}"

    val holders = Seq(
      DeclarationHolder(Some("authorisationTypeCodeOne"), Some("eoriOne"), Some(EoriSource.UserEori)),
      DeclarationHolder(Some("authorisationTypeCodeTwo"), Some("eoriTwo"), Some(EoriSource.OtherEori)),
      DeclarationHolder(Some("authorisationTypeCodeThree"), Some("eoriThree"), Some(EoriSource.UserEori))
    )

    "produce the expected ExportsDeclarationDiff instance" when {
      "no differences exist between the two versions" in {
        withClue("when no holders are present") {
          val declarationHolders = DeclarationHolders(Seq.empty, None)
        ...
        withClue("when holders are present") {
          val declarationHolders = DeclarationHolders(holders, Some(YesNoAnswer.yes))
          declarationHolders.createDiff(declarationHolders, Container.pointer, Some(1)) mustBe Seq.empty[AlteredField]
        }
      }
      ...
      "when holders are present but not equal" in {
        ...
        withClue("this DeclarationHolder's holders are not present") {
          val declarationHolders = DeclarationHolders(Seq.empty, Some(YesNoAnswer.yes))
        ...
        withClue("both DeclarationHolder's holders contain different number of elements") {
          val declarationHolders = DeclarationHolders(holders.drop(1), Some(YesNoAnswer.yes))
          declarationHolders.createDiff(declarationHolders.copy(holders = holders), baseFieldPointer, Some(1)) mustBe Seq(
        ...
        withClue("DeclarationHolder's holders contain elements with different values") {
          val otherHolder = DeclarationHolder(Some("authorisationTypeCodeOther"), Some("eoriOther"), Some(EoriSource.UserEori))
          val declarationHolders = DeclarationHolders(Seq(otherHolder) ++ holders.drop(1), Some(YesNoAnswer.yes))
          declarationHolders.createDiff(declarationHolders.copy(holders = holders), baseFieldPointer, Some(1)) mustBe Seq(
