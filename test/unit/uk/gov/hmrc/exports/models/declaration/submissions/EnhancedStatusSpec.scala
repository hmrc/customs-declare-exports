  ...
  "EnhancedStatus.apply" should {
    ...
    "return the expected EnhancedStatus enum" when {
      ...
      "the notification's status is CANCELLED and a successful cancellation request has NOT been already made" in {
        ...
        enhancedStatus(notification, List(CancellationAction("Some Id"))) mustBe EXPIRED_NO_DEPARTURE
      }
      ...
      "the notification's status is CANCELLED and a successful cancellation request has been already made" in {
        ...
        val action = CancellationAction("Some Id", notifications = notificationSumaries)
