    ...
    "produce the expected ExportsDeclarationDiff instance" when {
      "no differences exist between the two versions" in {
        withClue("all values are None") {
          val goodsLocation =
            GoodsLocation("latestCountry", "latestTypeOfLocation", "latestQualifierOfIdentification", Some("latestIdentificationOfLocation"))
      ...
      "the original version's identificationOfLocation field has a different value to this one" in {
        withClue("both version have Some value") {
          ...
          val goodsLocation =
            GoodsLocation("latestCountry", "latestTypeOfLocation", "latestQualifierOfIdentification", Some("latestIdentificationOfLocation"))
          val originalValue = "original"
          goodsLocation.createDiff(goodsLocation.copy(identificationOfLocation = Some(originalValue)), baseFieldPointer) mustBe Seq(
            constructAlteredField(fieldPointer, originalValue, goodsLocation.identificationOfLocation.get)
          )
        }

        withClue("original version has Some, this has None") {
          val fieldPointer = s"${baseFieldPointer}.${GoodsLocation.identificationOfLocationPointer}"
          val goodsLocation = GoodsLocation("latestCountry", "latestTypeOfLocation", "latestQualifierOfIdentification", None)
          val originalValue = Some("latestIdentificationOfLocation")
        ...
        withClue("original version has None, this has Some") {
          val fieldPointer = s"${baseFieldPointer}.${GoodsLocation.identificationOfLocationPointer}"
          val goodsLocation =
            GoodsLocation("latestCountry", "latestTypeOfLocation", "latestQualifierOfIdentification", Some("latestIdentificationOfLocation"))
          val originalValue = None
      ...
      "differences exist between the two versions" when {
        ...
        "the original version's officeOfExit field has a different value to this one" in {
          ...
          withClue("both versions have Some(Some) values but values are different") {
            val locations = Locations(officeOfExit = Some(OfficeOfExit(Some("latest"))))
            val originalValue = OfficeOfExit(Some("original"))
          ...
          withClue("the original version's officeOfExit field is Some(None) but this one has Some(Some) value") {
            val locations = Locations(officeOfExit = Some(OfficeOfExit(Some("latest"))))
            val originalValue = OfficeOfExit(None)
            locations.createDiff(locations.copy(officeOfExit = Some(originalValue)), baseFieldPointer) mustBe Seq(
              constructAlteredField(fieldPointer, originalValue, locations.officeOfExit.get)
            )
          }

          withClue("the original version's officeOfExit field is None but this one has Some(Some) value") {
            val locations = Locations(officeOfExit = Some(OfficeOfExit(Some("latest"))))
            val originalValue = None
            locations.createDiff(locations.copy(officeOfExit = originalValue), baseFieldPointer) mustBe Seq(
              constructAlteredField(fieldPointer, originalValue, locations.officeOfExit)
            )
          }

          withClue("the original version's officeOfExit field is None but this one has Some(None) as its value") {
            val locations = Locations(officeOfExit = Some(OfficeOfExit(None)))
            val originalValue = None
            locations.createDiff(locations.copy(officeOfExit = originalValue), baseFieldPointer) mustBe Seq(
              constructAlteredField(fieldPointer, originalValue, locations.officeOfExit)
            )
          ...
          withClue("the original version's officeOfExit field is Some(None) but this one has None as its value") {
            val locations = Locations(officeOfExit = None)
            val originalValue = Some(OfficeOfExit(None))
            locations.createDiff(locations.copy(officeOfExit = originalValue), baseFieldPointer) mustBe Seq(
              constructAlteredField(fieldPointer, originalValue, locations.officeOfExit)
            )
          }
