...
class TransportPaymentParserSpec extends UnitSpec {
  ...
  "TransportPaymentParser on parse" should {
    ...
    "return the expected PaymentMethod" when {
      "the '/ Consignment / Freight / PaymentMethodCode' element is present" in {
        ...
        val transportPayment = parser.parse(input).get
        transportPayment.paymentMethod mustBe TransportPayment.cash
      }
    }
  }

  private def inputXml(transportPayment: Option[String] = None): Elem =
    <meta>
      <ns3:Declaration>
        {transportPayment.map { tP =>
        <ns3:Consignment>
          <ns3:Freight>
            <ns3:PaymentMethodCode>{tP}</ns3:PaymentMethodCode>
          </ns3:Freight>
        </ns3:Consignment>
