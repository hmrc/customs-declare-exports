    ...
    "return Right with a List of PackageInformation instances" when {
      ...
      "multiple '/ GovernmentAgencyGoodsItem / Packaging' elements are fully present" in {
        ...
        listOfPackageInformation.head.get must matchPattern {
          case PackageInformation(_, Some("TypeCode1"), Some(1), Some("MarksNumbersID1")) =>
        }
        listOfPackageInformation.last.get must matchPattern {
          case PackageInformation(_, Some("TypeCode2"), Some(2), Some("MarksNumbersID2")) =>
        }
      }
