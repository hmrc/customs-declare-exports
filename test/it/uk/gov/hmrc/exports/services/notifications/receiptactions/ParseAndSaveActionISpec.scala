...
class ParseAndSaveActionISpec extends IntegrationTestBaseSpec {

  private val injector = GuiceApplicationBuilder().configure(mongoConfiguration).injector

  private val notificationFactory = injector.instanceOf[NotificationFactory]
  private val notificationRepository = injector.instanceOf[ParsedNotificationRepository]
  private val submissionRepository = injector.instanceOf[SubmissionRepository]

  private val parseAndSaveAction = new ParseAndSaveAction(submissionRepository, notificationRepository, notificationFactory)
    ...
    "provided with multiple ParsedNotifications with different actionIds but related to the same Submission, and" when {
      "the Submission's actions DO NOT CONTAIN yet any NotificationSummary" should {
        "return the Submission with the actions including the new NotificationSummaries in desc order" in {
          submissionRepository.save(submission.copy(actions = List(action_2, action_3))).futureValue
    ...
    "provided with multiple ParsedNotifications with different actionIds but related to the same Submission, and" when {
      "the Submission's actions DO CONTAIN already NotificationSummaries" should {
        "return the Submission with the actions including the new NotificationSummaries in desc order" in {
          ...
          submissionRepository.save(submission.copy(actions = List(action2, action3))).futureValue
    ...
    "provided with multiple ParsedNotifications with the same actionId, and" when {
      "the Submission's action DOES NOT CONTAIN yet any NotificationSummary" should {
        "return the Submission with the action including the new NotificationSummaries in desc order" in {
          submissionRepository.save(submission.copy(actions = List(action_2))).futureValue
    ...
    "provided with multiple ParsedNotifications with the same actionId, and" when {
      "the Submission's action DOES CONTAIN already NotificationSummaries" should {
        "return the Submission with the action including all NotificationSummaries in desc order" in {
          ...
          submissionRepository.save(submission.copy(actions = List(action2))).futureValue
