...
class RenameAndConvertBorderTransportNationalityFieldISpec @Inject() (countriesService: CountriesService) extends IntegrationTestMigrationToolSpec {
  ...
  "RenameAndConvertBorderTransportNationalityField migration" should {
    ...
    import RenameAndConvertBorderTransportNationalityFieldISpec._

    "correctly migrate declarations renaming countryName field and converting contents to country codes" in {
      runTest(declarationBeforeMigration1, declarationAfterMigration)
    }

    "rename countryName field but leave contents when already a country code " in {
      runTest(declarationBeforeMigration2, declarationAfterMigration)
    }

    "not alter documents already migrated" in {
      runTest(declarationAfterMigration, declarationAfterMigration)
    }
  }
}

object RenameAndConvertBorderTransportNationalityFieldISpec {

  val declarationBeforeMigration1 =
    """{
  ...
  val declarationBeforeMigration2 =
    """{
      |  "_id": { "$oid": "63f875806832ef2e3e4ed2ca" },
      |  "id": "c0305e9b-c48a-40cf-b74e-406c205f70db",
      |  "declarationMeta": {
