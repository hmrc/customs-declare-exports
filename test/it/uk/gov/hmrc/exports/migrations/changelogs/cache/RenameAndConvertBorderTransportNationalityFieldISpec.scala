...
class RenameAndConvertBorderTransportNationalityFieldISpec extends IntegrationTestMigrationToolSpec {
  ...
  "RenameAndConvertBorderTransportNationalityField migration" should {
    ...
    "rename countryName field but leave contents when already a country code " in {
      runTest(declarationBeforeMigrationWithCountryCode, declarationAfterMigration)
    }

    "rename field but log a warning about unmappable values (check manually)" in {
      runTest(declarationBeforeMigrationWithUnknownCountry, declarationAfterMigrationWithUnknownCountry)
    }
