...
class TransactionalOpsISpec extends IntegrationTestMongoSpec {
  ...
  private val transactionalOps = new TransactionalOps(mongoComponent, submissionRepository, notificationRepository)
  ...
  "TransactionalOps.updateSubmissionAndNotifications" when {
    ...
    "provided with a ParsedNotification without a stored Submission document (because it was removed in the meanwhile)" should {
