...
//TODO These component tests need fixing as the functionality / logic of the underlying code has changed
// dramatically but this test represent the logic / expected behaviour of the old code. Whilst I'm not a fan
// of commenting out test we ideally need this level / layer of testing so needs picking up and fixing by whomever
// has worked on the recent changes. Ideally this broken test should have been fixed at the time the code was changed.
